...
$query = "SELECT * FROM drivers WHERE is_deleted=0 ORDER BY firstname";
...
if ($users) {
	while ($user = mysqli_fetch_assoc($users)) {
		...
		$user_list .= "<td>{$user['licen_number']}</td>";
		$user_list .= "<td>{$user['work_experience']}</td>";
        $user_list .= "<td>{$user['working_time']}</td>";  
		$user_list .= "</tr>";
	}
} else {
...
<body style="overflow-x:hidden;">
	...
	<main style="margin-left: 20%;">
		...
		<table class="table table-success table-striped" style="width: 90%; margin-left: 4%; margin-top: 5%">
			<tr>
				...
				<th cope="col">Licen</th>
                <th cope="col">Work Experience</th>
                <th cope="col">Working Time</th>
			</tr>
